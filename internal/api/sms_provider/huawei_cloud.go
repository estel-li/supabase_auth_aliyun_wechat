...
func (p *HuaweiCloudProvider) VerifyOTP(phone string, otp string) error {
	// Huawei Cloud SMS doesn't support OTP verification directly
	// This method is required by the interface but not implemented
	return fmt.Errorf("OTP verification not supported by Huawei Cloud SMS provider")
}
